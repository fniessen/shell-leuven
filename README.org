...
ShellRC-Leuven is installed by running (one of) the following commands in your
terminal.
...
2. *Source ShellRC-Leuven configuration files*
   ...
   #+begin_src shell
   . ~/.shellrc-leuven/src/.bashrc
   #+end_src

   #+begin_src shell
   . ~/.shellrc-leuven/src/.zshrc
   #+end_src

   Or...

   #+begin_src shell :results silent
   if [ -d "$HOME/.shellrc-leuven/src" ]
   then
       PATH="$HOME/.shellrc-leuven/src:$PATH"
   fi
   #+end_src
...
## .commonshrc --- Common features between Bash and Z Shell
