...
=config-shell= contains miscellaneous settings common to Bash and Zsh.
...
* Common Configuration
:PROPERTIES:
:header-args+: :tangle ../../home/config-shell
:END:
...
#+begin_src shell :noweb yes
# Hey Emacs, this is a -*- sh -*- file
## config-shell --- Common features between Bash and Z Shell
...
- ~-R~ or ~--RAW-CONTROL-CHARS~ ::
     ...
     that option, ~git diff~ finally shows colors...
     #+end_note
...
#+begin_src shell :tangle ../../home/config-shell
...
alias reload='unalias -a && . "$HOME"/config-shell'
#+end_src
...
#+begin_src shell :tangle ~/.dotfiles/bin/svn-short-log
#!/usr/bin/awk -f
...
#+begin_src shell
alias svn-log="svn log | svn-short-log | sed -e 's/^\(.*\) | \(.*\) | \(.*\) | \(.*\)$/\o33\[0;33m\1\o33[0m \o33\[0;32m\2\o33[0m \4\o33\[0;34m<\3>/' | less -RF"
#+end_src

#+begin_src shell
alias svn-status="svn status | sed -e 's/^\(\([A-Z]\s\+\(+\s\+\)\?\)\?C .*\)$/\o33\[1;35m\1\o33[0m/' \
...
#+begin_src shell
# svn-changed release-number
alias svn-changed='svn log -v -r'
#+end_src

#+begin_src shell
# svn-filelog path/to/file
svn-filelog() {
