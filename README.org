...
* Important notice: repository obsolescence

The ~shell-leuven~ repository is now obsolete. For the latest updates, features,
and improvements, please refer to the new repository [[https://github.com/fniessen/shell-leuven-new][shell-leuven-new]].

The new repository contains all the enhancements and configurations from
~shell-leuven~ with added functionality and optimizations. Please migrate to this
new repo for continued support and updates.
...
experience on Windows (with WSL or Git BASH), Linux and Mac OS.
...
- (interactive) *login*
  ...
  (for which =.profile= is read, which sources =.bashrc= as well)
...
#+begin_src shell :tangle .bash_profile_leuven
...
One of the hardest things is to set the terminal window title "correctly".  Here
...
To find out if a given command exists (in the user's PATH), don't use ~which~
(because it is not in POSIX so its output format is unspecified, and it
complains when nothing is found instead of being silent):
...
Much like the ~PATH~ environment variable sets a search order for executable
commands, ~CDPATH~ allows you to define a selection of directories to be
automatically checked when accessing folders. When referencing a directory name,
the system will look for it in the directories specified in ~CDPATH~, streamlining
navigation *without requiring you to* type the full path or *use the ~cd~ command
explicitly*.
...
#+begin_src shell
# Check if the current operating system is WSL.
if [[ "$OSTYPE" = linux-gnu* ]]; then
    # Define aliases for changing to C: and D: drives.
    alias c:='cd /mnt/c'
    alias d:='cd /mnt/d'

    # Define aliases for opening files and URLs using Windows applications.
...
- sl :: "selected", which is used for highlighting the currently *matching line*.
- cx :: "context", which is used for highlighting context lines that are
  adjacent to matching lines.
- mt :: "match", which is used for highlighting the *matching pattern* itself. In
  this case, it is a *yellow text on a blue background* with bold and underline
  effects.
- ms :: "matching selected", which is used for highlighting matching text on the
  selected line.
- mc :: "matching context", which is used for highlighting matching text on
  context lines.
- fn :: "file name", which is used for highlighting file names in the output.
- ln :: "line number", which is used for highlighting line numbers in the
  output.
- bn :: "byte offset number", which is used for highlighting byte offset numbers
  in the output.
- se :: "separator", which is used for highlighting separator lines in the
  output.
...
The recommended way to do this is to add a custom ~db_home~ setting (comment the
example line) to =/etc/nsswitch.conf=.

#+begin_note
Editing your user's entry in the =/etc/passwd= file. Your ~HOME~ directory is the
penultimate element on your user's line (the ~HOME~ environment variable is set
from that field).
#+end_note
