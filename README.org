...
#+begin_src shell :tangle no
git clone https://github.com/karlin/mintty-colors-solarized.git ~/.dotfiles/plugins/mintty-colors-solarized
#+end_src
...
Get rid of my username and host in the agnoster theme.
...
if [ -r "$ZSH" ]; then
    build_prompt() {
        ...
        prompt_status
        prompt_virtualenv
        # prompt_context --> generates: username@hostname (don't want this)
        prompt_dir
        prompt_git
        prompt_svn
        prompt_end
