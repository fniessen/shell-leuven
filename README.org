...
- Utils
  ...
  + [X] ~cygutils~ or ~cygutils-extra~ (for ~unix2dos~)
  + [X] ~tig~ (Git browser)
  + [ ] ~fdupes~
...
for editor in emacsclientw emacsclient emacs; do
    if $(type $editor 2>&1 /dev/null); then
        # Set EDITOR.
        case $editor in
            emacsclient*)
...
#+begin_src shell
# My traceroute.
type mtr 2>&1 /dev/null && alias ping=mtr
...
type curl 2>&1 /dev/null && alias whatismyip='curl -s http://whatismyip.org'
#+end_src
